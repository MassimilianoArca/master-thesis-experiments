...
class SynthClassificationSimulationV2(Simulation):
    ...
    def generate_dataset(self, n_concepts, concept_size, last_concept_size):
        ...
        for class_index in range(self.generator.n_classes):
            ...
            class_perturbation_intensity = np.random.choice(PERTURBATION_INTENSITY, size=1)[0]

            for _ in range(3):

                perturbation = {}

                if class_perturbation_type == "mean" and class_perturbation_intensity == "small":
                    perturbation['type'] = 'mean'
                    perturbation['value'] = np.random.normal(scale=1, size=self.generator.size)

                elif class_perturbation_type == "mean" and class_perturbation_intensity == "large":
                    perturbation['type'] = 'mean'
                    perturbation['value'] = np.random.normal(scale=4, size=self.generator.size)

                elif class_perturbation_type == "combination" and class_perturbation_intensity == "small":
                    perturbation['type'] = 'combination'
                    perturbation['mean'] = np.random.normal(scale=1, size=self.generator.size)
                    ...
                    perturbation['type'] = 'combination'
                    perturbation['mean'] = np.random.normal(scale=4, size=self.generator.size)
        ...
        for i in range(n_concepts):
            ...
            for class_ in range(self.generator.n_classes):
                ...
                if concept_perturbation['type'] == 'mean':
                    self.generator.mean_values[class_] = self.mean_values[class_] + concept_perturbation['value']

                else:
                    self.generator.mean_values[class_] = self.mean_values[class_] + concept_perturbation['mean']
                    self.generator.cov_values[class_] = self.cov_values[class_] + concept_perturbation['cov']

                    dims = np.random.choice(
                        list(range(self.generator.size)), size=2, replace=False
                    )
                    self.generator.rotate(dims[0], dims[1], concept_perturbation['theta'], class_)
