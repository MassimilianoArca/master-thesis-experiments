...
def make_gaussian_binary_classification(n_samples=1000,
                                        n_features=5,
                                        n_informative=5,
                                        n_redundant=0,
                                        random_state=42,
...
if __name__ == '__main__':
    X, y = make_gaussian_binary_classification(n_samples=1000, n_features=3, n_informative=3, n_classes=3)

    columns = ['X_' + str(i) for i in range(X.shape[1])]
    columns.append('y_0')

    dataset = pd.DataFrame(
        np.column_stack((X, y)), columns=columns
    )
