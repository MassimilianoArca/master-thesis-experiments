...
class WeightedSamplingStrategy(BaseStrategy):
    ...
    def train_model(self):
        ...
        X = data[:, :-1]
        past_dataset.generated_dataset[
            past_dataset.generated_dataset.columns[:-1]
    ...
    def select_samples(self):
        ...
        if np.random.rand() < random_sampling_prob:
            ...
            selected_sample_index = np.random.choice(
                self.past_dataset.get_dataset().index.tolist()
            )
            sample = (
            ...
            if self.all_selected_samples:
                ...
                all_selected_samples = all_selected_samples[
                    all_selected_samples.columns[:-1]
                ]
                ...
                distance_matrix = 1 - pd.DataFrame(
                    pairwise.rbf_kernel(X=X, Y=all_selected_samples, gamma=0.1)
                )
